...
def test_one_text_interval__returns_text_interval():
  ...
  result = list(group_adjacent_intervals(intervals, set()))
  assert len(result) == 1
...
def test_one_pause_interval__returns_pause_group():
  ...
  result = list(group_adjacent_intervals(intervals, {""}))
