...
from textgrid_tools.app.helper import (add_n_digits_argument,
                                       ...
                                       save_grid)
from textgrid_tools.core.grid.interval_splitting import (
    can_split_grid_on_intervals, split_grid_on_intervals)
from tqdm import tqdm


def init_files_split_grid_on_intervals_parser(parser: ArgumentParser):
  ...
  parser.add_argument("--include-empty", action="store_true",
                      help="include empty intervals, i.e., those which contain no text or only whitespace")
  ...
  return files_split_grid_on_intervals


def files_split_grid_on_intervals(directory: Path, tier: str, audio_directory: Path, include_empty: bool, n_digits: int, output_directory: Optional[Path], output_audio_directory: Optional[Path], overwrite: bool) -> None:
  ...
  for file_stem in cast(Iterable[str], tqdm(common_files)):
    ...
    can_split = can_split_grid_on_intervals(grid_in, audio_in, sample_rate, tier)
    if not can_split:
      logger.info("Skipped.")
      continue

    try:
      grids_audios = split_grid_on_intervals(
          grid_in, audio_in, sample_rate, tier, include_empty, n_digits)
    except Exception:
      logger.info("Skipped.")
      continue
