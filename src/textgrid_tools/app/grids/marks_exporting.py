...
def get_marks_exporting_parser(parser: ArgumentParser):
  ...
  parser.add_argument("tier", type=parse_non_empty_or_whitespace,
                      help="tier containing the intervals that should be exported", action=ConvertToOrderedSetAction)
...
def app_plot_interval_durations(directory: Path, tier: str, output: Path, encoding: str, n_digits: int, overwrite: bool) -> ExecutionResult:
  ...
  (error, _), txt = get_marks_txt(grids, tier)
