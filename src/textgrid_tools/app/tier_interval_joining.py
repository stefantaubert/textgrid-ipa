...
from textgrid_tools.core.mfa.tier_interval_joining import (can_join_intervals,
                                                           join_intervals)
...
def files_join_intervals(grid_folder_in: Path, tier: str, new_tier: str, min_pause_s: float, overwrite_tier: bool, n_digits: int, grid_folder_out: Path, overwrite: bool) -> None:
  ...
  for file_stem in cast(Iterable[str], tqdm(grid_files)):
    ...
    can_join = can_join_intervals(grid_in, tier, new_tier, min_pause_s, overwrite_tier)
    if not can_join:
      logger.info("Skipped.")
      continue

    join_intervals(grid_in, tier, new_tier, min_pause_s, overwrite_tier)
