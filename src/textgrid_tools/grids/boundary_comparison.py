...
def compare_multiple_grids(grids: List[Tuple[TextGrid, TextGrid]], tier: str, ignore_marks: Set[str], limits_ms_excl: Set[float], extra_groups: Dict[str, List[str]], add_zero_diff: bool) -> Tuple[pd.DataFrame, Figure]:
  ...
  df = get_df_from_diffs(all_diffs, limits_ms_excl, durations1,
                         durations2, extra_groups, add_zero_diff)
...
def get_df_from_diffs(diffs: Dict[str, List[float]], limits_ms_excl: Set[float], durations1: Dict[str, List[float]], durations2: Dict[str, List[float]], extra_groups: Dict[str, List[str]], add_zero_diff: bool) -> pd.DataFrame:
  ...
  if len(limits_ms_excl) > 0:
    extended_limits = []
    if add_zero_diff:
      extended_limits.append(0)
    else:
      # zero is included in first limit-range
      extended_limits.append(-1)

    extended_limits.extend(sorted((limits_ms_excl - {0, math.inf})) + [math.inf])
  ...
  for mark in sorted_marks:
    ...
    row["MinDurA"] = np.min(durations1[mark]) if len(durations1[mark]) > 0 else np.nan
    row["MaxDurA"] = np.max(durations1[mark]) if len(durations1[mark]) > 0 else np.nan
    row["MeanDurA"] = np.mean(durations1[mark])
    row["MeanDurAStd"] = np.std(durations1[mark])
    row["MinDurB"] = np.min(durations2[mark]) if len(durations2[mark]) > 0 else np.nan
    row["MaxDurB"] = np.max(durations2[mark]) if len(durations2[mark]) > 0 else np.nan
    row["MeanDurB"] = np.mean(durations2[mark])
    row["MeanDurBStd"] = np.std(durations2[mark])
    ...
    if add_zero_diff:
      row["Match0ms"] = sum(1 for x in mark_diffs if x == 0)
    ...
    if add_zero_diff:
      row["Match0ms%"] = row["Match0ms"] / len(mark_diffs) * 100
  ...
  for group_name, group_marks in extra_groups.items():
    ...
    row["MinDurA"] = np.min([duration for m in unique_marks for duration in durations1[m]])
    row["MeanDurA"] = np.mean([duration for m in unique_marks for duration in durations1[m]])
    row["MeanDurAStd"] = np.std([duration for m in unique_marks for duration in durations1[m]])
    row["MaxDurA"] = np.max([duration for m in unique_marks for duration in durations1[m]])

    row["MinDurB"] = np.min([duration for m in unique_marks for duration in durations2[m]])
    row["MeanDurB"] = np.mean([duration for m in unique_marks for duration in durations2[m]])
    row["MeanDurBStd"] = np.std([duration for m in unique_marks for duration in durations2[m]])
    row["MaxDurB"] = np.max([duration for m in unique_marks for duration in durations2[m]])
    ...
    if add_zero_diff:
      row["Match0ms"] = np.sum([r["Match0ms"] for r in rs])
    for min_lim, max_lim in limits:
      row[f"Match({min_lim},{max_lim}]ms"] = np.sum(
        [r[f"Match({min_lim},{max_lim}]ms"] for r in rs])
    if add_zero_diff:
      row["Match0ms%"] = row["Match0ms"] / row["Occurrences"] * 100
    for min_lim, max_lim in limits:
      row[f"Match({min_lim},{max_lim}]ms%"] = row[f"Match({min_lim},{max_lim}]ms"] / \
        row["Occurrences"] * 100
...
def compare_grids(grid1: TextGrid, grid2: TextGrid, tier: str, ignore_marks: Set[str]) -> List[Tuple[str, float]]:
  ...
  for i1, i2 in zip(intervals1, intervals2):
    if i1.mark != i2.mark:
      raise Exception(
        "Mark mismatch! Both tiers need to have the same marks at the same positions.")
    mark = i1.mark
