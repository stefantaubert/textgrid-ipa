...
def app_split_intervals(directory: Path, tiers: OrderedSet[str], symbol: str, keep: bool, n_digits: int, output_directory: Optional[Path], n_jobs: int, chunksize: int, maxtasksperchild: Optional[int], log: Optional[Path], chunk: Optional[int]) -> ExecutionResult:
  method = partial(
    split_intervals,
    symbol=symbol,
    keep=keep,
  ...
  return process_grids_mp(directory, n_digits, output_directory, method, chunksize, n_jobs, maxtasksperchild, log, chunk)
